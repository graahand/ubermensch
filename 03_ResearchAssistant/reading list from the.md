1. **Jiafei Duan**
2. Jieyu Zhang
3. ZhongZheng Ren


## papers

### vlm and action theme

1. manipulate anything (2024, CoRL)
2. AHA, JIAFEI DUAN
3. Robopoint, jiafei duan 
4. task me anything, jieyu zhang 
5. molmoact, jieyu zhang.
6. CAPTURe, jaemin cho
7. paxion, jaemin cho
8. textless vision language transformer, jaemin cho
### data collection, synthesis and innovation theme 

1. datacomp, jieyu zhang
2. EnvGen, jaemin cho
3. provision, jieyu zhang
4. sugarcrepe, jieyu zhang
5. ar2-d2, jiafei duan 


### 3d and vlm grounding

1. redo, zhongzheng ren
2. 3d spatial recognition without spatially labelled 3d, zhongzheng ren
3. NeRFDeformer, zhongzheng ren 

### autonomous vehicles and security
1. doublestar, ce zhou
2. transient adversarial 3d projection attackes on object detection in autonomous driving, ce zhou
3. short: breaking the charge, ce zhou 
4. 